...
const CONSECUTIVE_MARGIN = 12; // hours
...
(document.getElementById("save-shift") as HTMLDivElement).onclick = () => {
  ...
  if (end.getTime() < start.getTime()) {
    end.setDate(end.getDate() + 1); // If end time is before start time, assume it is the next day
  }
  const employee = (
...
(document.getElementById("save-employee") as HTMLDivElement).onclick = () => {
  ...
  if (!isNaN(hours)) selected.employee.desiredHours = hours;
  const msg: clientMessage = {
    type: "changeEmployee",
...
function populateEmployees() {
  ...
  employees.forEach((employee, employeeIndex) => {
    ...
    const showHideInfo = document.createElement("span");
    showHideInfo.className = "show-hide-info";
    showHideInfo.innerHTML = "&#9654;&nbsp;";
    const detailsDiv = document.createElement("div");
    detailsDiv.style.display = "none";
    showHideInfo.onclick = (e) => {
      e.stopPropagation();
      e.preventDefault();
      if (showHideInfo.classList.contains("showing")) {
        showHideInfo.classList.remove("showing");
        showHideInfo.innerHTML = "&#9654;&nbsp;";
        detailsDiv.style.display = "none";
      } else {
        showHideInfo.classList.add("showing");
        showHideInfo.innerHTML = "&#9660;&nbsp;";
        detailsDiv.style.display = "";
      }
    };
    showHideInfo.ondblclick = (e) => {
      e.stopPropagation();
      e.preventDefault();
    };
    employeeNameSpan.appendChild(showHideInfo);
    const employeeNameSpanSpan = document.createElement("span");
    employeeNameSpanSpan.innerHTML = employee.name;
    employeeNameSpan.appendChild(employeeNameSpanSpan);
    employeeName.appendChild(employeeNameSpan);
    employeeEl.appendChild(employeeName);
    employeeEl.appendChild(detailsDiv);
    if (weekIndex >= 0 && weeks[weekIndex]) {
      ...
      if (hasConsecutiveShifts) {
        ...
        employeeName.style.color = "#f00";
        detailsDiv.appendChild(consecutiveDiv);
      }
      const totalSpan = document.createElement("span");
      let percentage: number | null = null;
      if (employee.desiredHours) {
        const totalHours = totalsObj[""];
        percentage = (100 * totalHours) / employee.desiredHours;
        percentage = Math.round(percentage);
        totalSpan.innerHTML = `${totalHours} (${percentage.toFixed(2)}%)`;
        if (Math.abs(percentage - 100) > 10) {
          totalSpan.style.color = "#ff0";
        }
        if (Math.abs(percentage - 100) > 20) {
          totalSpan.style.color = "#f00";
        }
      }

      totalSpan.innerHTML = percentage === null ? "" : percentage + "%";
...
function isWithinHours(
  shift1: Shift,
  shift2: Shift,
  makePreviousDay?: boolean
): boolean {
  //console.log("isWithinHours", shift1, shift2, makePreviousDay);
  const start1 = new Date(shift1.start);
  start1.setUTCFullYear(shift2.start.getUTCFullYear());
  start1.setUTCMonth(shift2.start.getUTCMonth());
  start1.setUTCDate(shift2.start.getUTCDate());
  if (makePreviousDay) {
    start1.setDate(start1.getDate() - 1);
  }
  const end1 = new Date(
    start1.getTime() + (shift1.end.getTime() - shift1.start.getTime())
  );
  //console.log(`start1: ${start1.toISOString()}, end1: ${end1.toISOString()}`);
  //console.log("length");
  //console.log(start1.getTime() + (shift1.end.getTime() - shift1.start.getTime()));
  const start2 = new Date(shift2.start);
  const end2 = new Date(shift2.end);
  //console.log(`start1: ${start1.toISOString()}, end1: ${end1.toISOString()}, start2: ${start2.toISOString()}, end2: ${end2.toISOString()}`);
  if (start1 <= end2 && end1 >= start2) {
    //console.log("shifts overlap: true");
    return true;
  }
  const msDiff = Math.min(
  ...
  if (msDiff <= msThreshold) {
    ...
    //console.log("shifts outside margin: false");
  }
  return msDiff <= msThreshold;
}
...
function hasNeighboringShift(
  ...
  //console.log(employee, dayIndex);
  const neighborShifts: Shift[] = [];
  if (weekIndex >= 0) {
    const day = weeks[weekIndex][dayIndex];
    day.shifts.forEach((sameDayShift, index) => {
      //console.log("sameDayShift", dayIndex);
      if (shiftIndex !== index && isWithinHours(shift, sameDayShift)) {
        //console.log("pushing sameDayShift");
        neighborShifts.push(sameDayShift);
      }
    });
    ...
    if (previousDay) {
      previousDay.shifts.forEach((previousDayShift) => {
        //console.log("previousDayShift", dayIndex);
        if (isWithinHours(previousDayShift, shift, true)) {
          //console.log("pushing previousDayShift");
          neighborShifts.push(previousDayShift);
        }
      });
    }
    ...
    if (nextDay) {
      nextDay.shifts.forEach((nextDayShift) => {
        //console.log("nextDayShift", dayIndex);
        if (isWithinHours(shift, nextDayShift, true)) {
          //console.log("pushing nextDayShift");
          neighborShifts.push(nextDayShift);
        }
      });
    }
  }
  ...
  neighborShifts.forEach((neighborShift) => {
    if (neighborShift.employee === employee) {
      ...
      //console.log(`neighbor shift employee does not match "${employee}", "${neighborShift.employee}"`);
      //console.log(neighborShift);
    }
  });
  //console.log("hasNeighboringShift", rtn);
  return rtn;
}
...
function isRecommended(
  ...
  if (
    ...
    //console.log("not recommended: has neighboring shift");
    return false;
  }
  ...
  employee.unavailable.forEach((unavailable) => {
    ...
    if (unavailable.day === shiftDay + 1 || unavailable.day === shiftDay - 6) {
      const unavailableStart = new Date(unavailable.start.getTime());
      unavailableStart.setDate(unavailableStart.getDate() + 1);
      const unavailableEnd = new Date(unavailable.end.getTime());
      unavailableEnd.setDate(unavailableEnd.getDate() + 1);
      if (
        (shift.start >= unavailableStart && shift.start <= unavailableEnd) ||
        (shift.end >= unavailableStart && shift.end <= unavailableEnd)
      ) {
        rtn = false;
        return false;
      }
