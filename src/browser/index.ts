...
function populateWeeks() {
  ...
  week.forEach((day: Day | DefaultDay, dayIndex) => {
    ...
    day.shifts.forEach((shift, shiftIndex) => {
      ...
      if (shift.position) shiftDiv.classList.add(shift.position.replace(/\s+/g, "-").toLowerCase());
...
function isWithinHours(
  ...
  const endTime1 = end1.getTime() < start1.getTime() ? end1.getTime() + 24 * 60 * 60 * 1000 : end1.getTime();
  const endTime2 = end2.getTime() < start2.getTime() ? end2.getTime() + 24 * 60 * 60 * 1000 : end2.getTime();
